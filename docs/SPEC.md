...
| **安全な書き込み順序** | 非冗長ゾーンは遅延割り当て (LVM allocatable=n) し、冗長ゾーンから先に使う |
...
### 3.5 遅延割り当て (Deferred Allocation)

非冗長ゾーン (SINGLE) にはデータを後回しで書き込む。
LVM の `pvchange -x` (allocatable フラグ) を利用。
...
ディスク追加で非冗長ゾーンが発生:
  1. mdadm アレイ作成                     → 通常通り
  2. pvcreate → vgextend                  → VG に追加
  3. pvchange -x n                        → 割り当て禁止 ★
  4. LV は冗長ゾーンの PV のみを使用

冗長ゾーンの使用率がしきい値 (90%) 超過:
  1. monitor が検知 → ログ出力 + webhook 通知
  2. ユーザーが手動で有効化:
     $ puddle expand-unprotected --yes
  3. pvchange -x y → lvextend → resize2fs
  4. status に非冗長使用中の警告を常時表示

新ディスク追加で全ゾーン冗長化:
  1. SINGLE ゾーンが RAID1/5 に昇格
  2. allocatable フラグは自動で y に変更
  3. 警告解消
```

#### 3.5.2 メタデータ

ZoneMeta に `allocatable` フィールドを追加:

```toml
[[zones]]
index = 2
raid_level = "single"
allocatable = false   # LVM 割り当て禁止
```

- 冗長ゾーン (RAID1/5/6): `allocatable = true` (デフォルト)
- 非冗長ゾーン (SINGLE、かつプール内に2台以上のディスクがある場合): `allocatable = false`
- init 直後の1台構成: `allocatable = true` (使わないと何も保存できない)

#### 3.5.3 CLI: `puddle expand-unprotected`

```bash
$ puddle expand-unprotected
⚠ WARNING: 非冗長領域 (Zone 2: 2.0 TB, SINGLE) をデータ書き込み対象に含めます。
  この領域はディスク障害時にデータを失う可能性があります。

  冗長領域: 10.0 TB (90% 使用中)
  追加される非冗長領域: 2.0 TB

Proceed? [Y/n] y
  Enabling allocation on Zone 2...     done
  Extending logical volume...          done
  Resizing filesystem...               done
✓ Non-redundant storage enabled. Total capacity: 12.0 TB

⚠ puddle status will show a persistent warning while unprotected storage is in use.
  Add another disk (4TB+) to protect this zone.
```
...
Capacity:
  Protected:   10.0 TB
  Unprotected:  2.0 TB (reserved)
  Used:         1.2 TB (12% of protected)
  Free:         8.8 TB
```
...
```
Zones:
  Zone 2  SINGLE  1 disk  × 2.0 TB  [active — NO REDUNDANCY] ⚠

Capacity:
  Protected:   10.0 TB
  Unprotected:  2.0 TB (IN USE — no redundancy) ⚠
  Used:        10.5 TB
  Free:         1.5 TB
```
...
Planning zone layout...
  ...
  New layout:
    ...
    Zone 1: SINGLE (1 disk × 2TB) → reserved (no redundancy) ⚠

  Effective capacity: 2TB → 2TB (protected)
  Reserved (unprotected): 2TB

Proceed? [Y/n] y
  Creating partitions on /dev/sdc...       done
  Creating RAID array puddle-z0...         done (syncing in background)
  Creating RAID array puddle-z1...         done (reserved, allocation disabled)
  Extending volume group...                done
  Extending logical volume...              done
  Resizing filesystem...                   done
✓ Disk added successfully. RAID sync: 15% (ETA: 3h 20m)
ℹ Zone 1 (2TB) is reserved. It will be used when protected space is full
  or when you run 'puddle expand-unprotected'.
...
$ puddle monitor --webhook https://hooks.slack.com/services/...
# 継続監視 + webhook 通知
